...
class AnnotatePageManager {
    ...
    initializeAnnotationEngine() {
        ...
        this.annotationEngine.on('zoomChanged', this.handleZoomChanged.bind(this));
    ...
    initializeCanvasControls() {
        ...
        if (fitScreenBtn) {
            fitScreenBtn.addEventListener('click', () => {
                if (this.annotationEngine) {
                    this.annotationEngine.fitToScreen();
                }
            });
        }
        ...
        if (zoomSlider) {
            zoomSlider.addEventListener('input', (e) => {
                if (this.annotationEngine) {
                    ...
                    this.annotationEngine.setZoom(scale);
                }
            });
        }

        // Grid and crosshair toggles
        ...
        if (toggleGridBtn) {
            toggleGridBtn.addEventListener('click', () => {
                if (this.annotationEngine) {
                    this.annotationEngine.toggleGrid();
        ...
        if (toggleCrosshairBtn) {
            toggleCrosshairBtn.addEventListener('click', () => {
                if (this.annotationEngine) {
                    this.annotationEngine.toggleCrosshair();
                    toggleCrosshairBtn.classList.toggle('active');
                }
            });
        }
     ...
     */
    setTool(tool) {
        if (!this.annotationEngine) return;
        ...
        switch (tool) {
            case 'select':
                ...
                break;
            case 'rectangle':
                this.annotationEngine.setTool('rectangle');
                break;
            case 'polygon':
                this.annotationEngine.setTool('polygon');
                break;
            case 'zoom-in':
                this.annotationEngine.zoomIn();
                break;
            ...
            case 'redo':
                ...
                break;
            case 'delete':
                this.annotationEngine.deleteSelected();
                break;
        }
    }
    ...
    handleZoomChanged(event) {
        ...
        if (canvasSizeElement && this.annotationEngine?.image) {
            const width = Math.round(this.annotationEngine.image.width * scale);
            const height = Math.round(this.annotationEngine.image.height * scale);
            canvasSizeElement.textContent = `${width} × ${height}`;
        }
    }
     ...
     */
    onPageShow() {
        console.log('Annotation page shown');
        ...
        if (currentDocumentId && currentDocumentId !== this.currentDocument?.id) {
            ...
            console.log('No document to load or already loaded');
        }
    }
