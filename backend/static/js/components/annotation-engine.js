...
class AnnotationEngine {
     ...
     */
    handleSelectMouseDown(pos, imagePos, event) {
    ...
    /**
     * Handle polygon point
     */
    handlePolygonPoint(pos, imagePos) {
        // TODO: Implement polygon annotation
        console.log('Polygon tool not yet implemented');
    }
     ...
     * Handle delete click
     */
    handleDeleteClick(pos, imagePos) {
        const annotation = this.getAnnotationAt(imagePos);
        if (annotation) {
    ...
    /**
     * Update annotation drag
     */
    updateAnnotationDrag(pos, imagePos) {
        if (!this.dragAnnotation || !this.dragStart) return;

        const deltaX = (pos.x - this.dragStart.x) / this.imageScale;
        const deltaY = (pos.y - this.dragStart.y) / this.imageScale;

        this.updateAnnotation(this.dragAnnotation.id, {
            x: this.dragAnnotation.x + deltaX,
            y: this.dragAnnotation.y + deltaY
        });
    ...
    /**
     * Finish current annotation (generic)
     */
    finishCurrentAnnotation() {
        if (this.currentTool === TOOL_MODES.RECTANGLE && this.currentAnnotation) {
            this.finishRectangleAnnotation();
        } else if (this.currentTool === TOOL_MODES.POLYGON && this.polygonPoints.length > 2) {
            this.finishPolygonAnnotation();
        }
    }
    ...
    finishPolygonAnnotation() {
        ...
        console.log('Polygon annotation completion not yet implemented');
        this.polygonPoints = [];
        this.isDrawing = false;
    }
    ...
    zoomIn(centerPoint = null) {
        const newScale = Math.min(this.imageScale + this.scaleStep, this.maxScale);
        this.setZoom(newScale, centerPoint);
    }

    zoomOut(centerPoint = null) {
        const newScale = Math.max(this.imageScale - this.scaleStep, this.minScale);
        this.setZoom(newScale, centerPoint);
    }

    setZoom(scale, centerPoint = null) {
        const oldScale = this.imageScale;
        this.imageScale = Math.max(this.minScale, Math.min(this.maxScale, scale));
        this.zoomLevel = this.imageScale;

        // Adjust pan offset to zoom towards center point
        if (centerPoint && this.image) {
            const scaleRatio = this.imageScale / oldScale;
            const canvasCenter = {
                x: this.canvas.width / 2,
                y: this.canvas.height / 2
            };

            const zoomCenter = centerPoint || canvasCenter;
        ...
        this.updateZoomUI();
        this.render();
        this.events.emit('zoomChanged', { scale: this.imageScale, level: this.zoomLevel });
    }
    ...
    fitToScreen() {
        ...
        const scaleX = (canvasRect.width - padding * 2) / this.image.width;
        const scaleY = (canvasRect.height - padding * 2) / this.image.height;
    ...
    centerImage() {
        ...
        const canvasRect = this.canvas.getBoundingClientRect();
        this.panOffset.x = (canvasRect.width - this.image.width * this.imageScale) / 2;
        this.panOffset.y = (canvasRect.height - this.image.height * this.imageScale) / 2;

        this.render();
    }

    /**
     * Enhanced pan functionality with smooth scrolling
     */
    smoothPanTo(targetX, targetY, duration = 300) {
        if (!this.smoothPan) {
        ...
        const animate = (currentTime) => {
            ...
            this.panOffset.x = startX + deltaX * easeOut;
            this.panOffset.y = startY + deltaY * easeOut;
    ...
    toggleGrid() {
        ...
        if (gridElement) {
            gridElement.classList.toggle('visible', this.showGrid);
        }
        this.render();
    }
    ...
    toggleCrosshair() {
        ...
        const crosshairElement = document.getElementById('canvas-crosshair');
        if (crosshairElement) {
            crosshairElement.classList.toggle('visible', this.showCrosshair);
        }
    }
    ...
    updateZoomUI() {
        ...
        if (zoomSlider) {
            zoomSlider.value = this.imageScale * 100;
        }

        if (zoomValue) {
            zoomValue.textContent = `${Math.round(this.imageScale * 100)}%`;
        }

        if (zoomLevel) {
            zoomLevel.textContent = `${Math.round(this.imageScale * 100)}%`;
        }
    }
    ...
    updateMousePosition(canvasPos) {
        this.mousePosition = canvasPos;
        const imagePos = this.canvasToImage(canvasPos);

        const positionElement = document.getElementById('mouse-position');
        if (positionElement) {
            positionElement.textContent = `${Math.round(imagePos.x)}, ${Math.round(imagePos.y)}`;
        }
        ...
        if (this.showCrosshair) {
            const crosshairElement = document.getElementById('canvas-crosshair');
            if (crosshairElement) {
                const crosshairH = crosshairElement.querySelector('.crosshair-h');
                const crosshairV = crosshairElement.querySelector('.crosshair-v');

                if (crosshairH) crosshairH.style.top = `${canvasPos.y}px`;
                if (crosshairV) crosshairV.style.left = `${canvasPos.x}px`;
            }
        }
    }
